...
<div class="container">
  ...
  <nav>
    <ul>
      <li><a href="./scheduleappointment.php">Schedule an Appointment</a></li>
      <li style="font-weight:bold">  Check Appointment </li>
  ...
  <article class="content">
	...
	if((isset($_POST['studentid'])) && (isset($_POST['dob']))){
		...
		if($rowcnt==0){
			?>
            <h2>Login</h2>
             <form action="checkappointment.php?c=1" method="post">
			...
			$htmlcode="<section><h3>Your Next Appointments-</h3><table border='0' cellspacing='0' cellpadding='2'>";
			
			$sql1 = "select s.student_id, s.student_dob, a.appointment_date, a.appointment_time from student as s, appointment as a where s.student_id = a.student_id_s and s.student_id= ".$_POST['studentid']." and s.student_dob = '".$_POST['dob']."'";
			$result1 = db($sql1);
			$rowcnt1 =  rowcount($sql1);
			if($rowcnt1 > 0){
				while($row = $result1->fetch_assoc()){
					$htmlcode = $htmlcode."<tr>
            <td >Appointment :-</td>
            <td> ".$row['appointment_date']."</td><td>".$row['appointment_time']."</td></tr>";
				}
				
			}else{
				$htmlcode = $htmlcode . "<tr><td> You have no future appointments </td></tr>";
			}
