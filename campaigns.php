...
<div class="container">
  ...
  <nav>
    ...
    <aside>
      <p>Here you can start new campaigns.(The email template is not yet made so this will just send a default test email to the list of students.)</p>
    </aside>
  <!-- end .sidebar1 --></div>
...
if(  !class_exists('db') ) {
    require_once('./db/db.php');
       ...
       if(!isset($_POST['viewname'])){
			...
			$result1 = db($str1);
			$htmlcode = "<section ><form action='./campaigns.php?p=1' method='post'> <select name='viewname'>";
			...
			$htmlcode = $htmlcode ."<select><input type='submit' name='submit1'></form></section>";
			  ...
			  $htmlcode = "<section >";
			  $str4 = "select student_email FROM ".$_POST['viewname'];
			  $result4 = db($str4);
			  if($result4->num_rows > 0){
			  	while($row4 = $result4->fetch_assoc()){
					mailer($row4['student_email'],"Test Mail");
				}
			  }
			  $htmlcode = $htmlcode ."<h3>Email has been sent to " . $_POST['viewname'] . " group. </hr> </section>";
